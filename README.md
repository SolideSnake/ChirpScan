...
X 的主贴时间线和回复时间线不是同一个接口。主贴通常走 `UserTweets`，回复优先走 `UserTweetsAndReplies`；当 X 前端接口、请求签名或 twikit 解析逻辑变化时，回复接口可能返回 `404`。

ChirpScan 的处理策略：

- `tweets` 只请求主贴接口，稳定性最高。
- `replies` 会优先请求回复时间线，失败后尝试用搜索兜底抓 `from:用户名 filter:replies`。
- `tweets_and_replies` 会先尝试主贴 + 回复；如果回复兜底也不可用，会临时退回只抓主贴，避免整个目标停止监控。
- 相关日志会显示“回复时间线接口暂不可用”“搜索兜底也不可用”等提示，这类日志表示 X 抓取侧波动，不代表电报、飞书或币安广场发送失败。
