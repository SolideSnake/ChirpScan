...
ChirpScan 是一个本地运行的 X 监控与平台推送工具。2.0 版本把目标管理统一到 `X 监控` 页面，并新增 `币安广场` 发布能力；当前支持把同一条推文按目标规则推送到 `电报` 和 `币安广场`。

## 2.0 主要变化

- 新增币安广场发布模块，支持按目标单独开启或关闭。
- 目标配置统一为 `MONITOR_TARGETS`，一个 X 用户下分别配置电报和币安广场规则。
- 电报和币安广场各自拥有独立关键词规则，互不影响。
...
- 目标管理：在 `X 监控` 页面新增 X 用户，并为每个目标选择电报、币安广场或两者同时推送。
...
- `设置`：配置 X Cookies、轮询间隔、电报 Bot、币安广场 API Key、重试参数和本地状态文件。
